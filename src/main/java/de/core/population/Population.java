...
import de.core.species.Species;
import de.core.species.SpeciesFitnessComparator;
...
public class Population {
	...
	public float highestAchievedFitness;
	...
	private void assignGenomeToSpecies() {
		...
		for (ArtificialIntelligence ai : this.artificialIntelligences) {
			...
			for (Species species : this.species) {
				...
				if (speciation.compatibilityDistance(PopulationConstants.C1, PopulationConstants.C2, PopulationConstants.C3) < PopulationConstants.DT) {
	...
	private void cullSpecies() {
		...
		float averageSum = this.getAverageFitnessSum();
	...
	private void calculateFitness() {
		...
		for (ArtificialIntelligence ai : this.artificialIntelligences) {
			float fitness = ai.calculateFitness();
	...
	private void breedNewGeneration() {
		...
		float averageFitnessSum = this.getAverageFitnessSum();
...
//			System.out.println("CHILDREN ALLOWED : " + allowedChildrenCount + " ## AVG SPECIES FITNESS: " + species.averageFitness + " ## AVG FITNES SUM : " + averageFitnessSum);
	 ...
	 * @return
	 */
	private float getAverageFitnessSum() {
